...
class _CertificateListScreenState extends State<CertificateListScreen> {
  ...
  Future<void> _loadCertificates() async {
    ...
    try {
      _certificates = await _repository.listCertificates({'query': _searchQuery});
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Certificate Registry'),
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text('Owner: ${cert.issuedByName}'), // In real app, fetch owner
            const SizedBox(height: 8),
            Row(
              children: [
                _buildStatusBadge(cert.status),
                const SizedBox(width: 8),
                Text('Valid Until: ${cert.validTo.substring(0, 10)}', style: const TextStyle(fontSize: 12)),
              ],
            ),
          ],
        ),
