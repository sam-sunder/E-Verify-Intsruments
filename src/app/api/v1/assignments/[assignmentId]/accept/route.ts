...
export const POST = routeHandler(async (_request, context) => {
  ...
  return ok(await officerAssignmentService.transition(principal, assignmentId, [AssignmentStatus.ASSIGNED], AssignmentStatus.ACCEPTED, { acceptedAt: new Date() }, "Assignment accepted"));
});